...
describe Bosh::CloudStackCloud::Cloud, "create_vm" do
  ...
  def server_params(unique_name, user_data, security_groups=[])
    {
  ...
  it "creates an CloudStack server and polls until it's ready(no disk)" do
    ...
    server = double("server", :id => "i-test", :name => "i-test", :display_name => "i-test")
    image = double("image", :id => "sc-id", :name => "sc-id")
    flavor = double("flavor", :id => "f-test", :name => "Medium Instance")
    address = double("address", :id => "a-test", :ip => "10.0.0.1",
                     :instance_id => "i-test")

    cloud = mock_cloud do |cloudstack|
      cloudstack.servers.should_receive(:create).
          with(server_params(unique_name, user_data, %w[default])).
          and_return(server)
      cloudstack.images.should_receive(:get).with("sc-id").and_return(image)
      cloudstack.flavors.should_receive(:find).and_return(flavor)
      cloudstack.addresses.should_receive(:each).and_yield(address)
    end
